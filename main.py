...
@app.get("/")
async def root():
    return {"message": "Hello Samhit"}


@app.get("/hello/{name}")
async def say_hello(name: str):
    return {"message": f"Awesome cloud developer sb4845 says hello {name}"}


@app.get("/hello_text/{name}")
async def say_hello_text(name: str):
    the_message = f"Awesome cloud developer sb4845 says Hello {name}"
